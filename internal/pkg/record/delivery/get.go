...
func readMultipartDataImages(r *http.Request) ([]domain.RecordImageInfo, error, int) {
	...
	for i, _ := range files { // loop through the files one by one
		...
		if err != nil {
			...
			return []domain.RecordImageInfo{}, domain.Err.ErrObj.InternalServer, http.StatusInternalServerError
		}

		if !validExtenstions(files) {
			return []domain.RecordImageInfo{}, domain.Err.ErrObj.BadFileExtension, http.StatusBadRequest
		}
		// TODO: parse tags
		// tags := make([]string, 0)
		...
		// }
		if err != nil {
			return []domain.RecordImageInfo{}, domain.Err.ErrObj.BadInput, http.StatusBadRequest
		}
...
func (handler *RecordHandler) CreateMedicRecord(w http.ResponseWriter, r *http.Request) {
	...
	readedImages, err, httpCode := readMultipartDataImages(r)
