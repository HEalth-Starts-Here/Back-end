...
const (
	...
	SELECT convert_from(decrypt(medicid::text::bytea,'%s','aes'),'SQL_ASCII')::bigint
	FROM diaries
	WHERE id = $1;
	`

	queryGetMedicAndPatientIdsFromDiaryOfRecord = `
	SELECT convert_from(decrypt(medicid::text::bytea,'%s','aes'),'SQL_ASCII')::bigint, convert_from(decrypt(patientid::text::bytea,'%s','aes'),'SQL_ASCII')::bigint
	FROM diaries
	JOIN medicrecords
